...
// others can only see their own company_id
function getAuthorizedCompanyId(user, requestedCompanyId) {
  if (user.role === 'admin') {
    return requestedCompanyId ? parseInt(requestedCompanyId) : null;
  }
  // Non-admin: always force their own company_id
  return parseInt(user.company_id);
}

async function assertInvoiceAccess(invoiceId, user) {
  const result = await query('SELECT id, company_id FROM ar_invoices WHERE id = $1', [invoiceId]);
  if (!result.rows[0]) return { error: 'not_found', message: 'Invoice not found.' };

  const invoice = result.rows[0];
  if (user.role !== 'admin' && invoice.company_id !== parseInt(user.company_id)) {
    return { error: 'forbidden', message: 'Access denied to this invoice.' };
  }
  return { invoice: result.rows[0] };
}

// ─── HELPERS ─────────────────────────────────────────────────
function buildFilters(params, authorizedCompanyId) {
  const conditions = [];
  const values = [];
  let idx = 1;
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { where, values, nextIdx } = buildFilters(req.query, authorizedCompanyId);
    ...
    const [invoices, summary, aging, total] = await Promise.all([
      ...
      query(`
        ...
        FROM ar_invoices i
        ${where} ${where ? 'AND' : 'WHERE'} outstanding_balance > 0 AND status NOT IN ('paid','cancelled')
      `, values),
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    // Assert access
    const access = await assertInvoiceAccess(id, req.user);
    if (access.error) {
...
router.post('/', async (req, res, next) => {
  try {
    ...
    `, [
      ...
      issue_date, due_date, cfdi_uuid || null, cfdi_xml_url || null,
      created_by
    ]);
...
router.post('/mark-overdue', async (req, res, next) => {
  try {
    // Only admin or finance can run overdue engine
    ...
    // Scope to user company unless admin
    const companyFilter = req.user.role !== 'admin'
      ? `AND company_id = ${parseInt(req.user.company_id)}`
      : '';
    ...
    const result = await query(`
      ...
      WHERE due_date < CURRENT_DATE
        ...
        ${companyFilter}
      RETURNING id, folio, due_date, outstanding_balance, company_id
    `);
