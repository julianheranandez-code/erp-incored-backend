...
router.post('/', async (req, res, next) => {
  try {
    ...
    const countResult = await query(
      `SELECT COALESCE(MAX(CAST(SPLIT_PART(folio, '-', 4) AS INTEGER)), 0) as cnt FROM ar_invoices WHERE company_id=$1 AND TO_CHAR(issue_date, 'MMYY') = $2`, [parseInt(company_id), mm+yy]
    );
...
router.post('/:id/submit', async (req, res, next) => {
  try {
    ...
    const approvalPolicy = getCompanyApprovalPolicy(invoice.company_id);
    let chain;
    try {
